...
const MF_LOCAL_RECORDS=foods.map(mfLocalRecord).map(food=>MF_CURATED_OVERRIDES[food.name]?{...food,...MF_CURATED_OVERRIDES[food.name]}:food);
...
function addWater(ml){state.waterLog.push({ml,date:today(),time:new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})});save();updateDashboard();renderWater();toast(`+${ml} ml water`)}
...
async function mfSearchGlobal(query) {
  ...
  mfRenderSearchStatus(
    `Searching globally for “${cleaned}”…`,
    true
  );
  ...
  /*
    IMPORTANT:
    v2 /api/v2/search is NOT a full-text endpoint.
    Search-a-licious /search is the current full-text service.
    We use its GET endpoint first to avoid a browser CORS
    preflight, then fall back to the legacy v1 full-text endpoint.
  */
  const searchAliciousUrl =
    "https://search.openfoodfacts.org/search" +
    "?q=" + encodeURIComponent(cleaned) +
    "&langs=en" +
    "&page=1" +
    "&page_size=" + MF_SEARCH.pageSize +
    "&boost_phrase=true" +
  ...
  const legacyUrl =
    ...
    "&json=1" +
    "&search_simple=1" +
    "&search_terms=" + encodeURIComponent(cleaned) +
    "&page_size=" + MF_SEARCH.pageSize +
    "&lc=en" +
    "&fields=" + encodeURIComponent(fields.join(","));
  ...
  async function getJson(url) {
    const response = await fetch(
      url,
      {
        method: "GET",
        mode: "cors",
        signal: MF_SEARCH.controller.signal,
        headers: {
  ...
  try {
    ...
    const products =
      mfExtractProducts(data);

    const ranked =
      products
        .filter(mfHasNutrition)
        .map(product => ({
          product,
          score:
            mfScoreProduct(
    ...
    for (const item of ranked) {
      ...
      const nutrition =
        mfNutrition(item.product);

      const key =
        `${mfNormalize(name)}|${item.product.code || ""}`;
    ...
    if (exact.length) {
      chunks.push(`
        <div class="result-section-title mf-global-heading">
          <span class="pill">
            GLOBAL · EXACT MATCHES · ${exact.length}
          </span>
        </div>
      `);
    ...
    if (related.length) {
      chunks.push(`
        <div class="result-section-title mf-global-heading">
          <span class="pill">
            GLOBAL · OTHER MATCHES · ${Math.min(related.length, 12)}
          </span>
        </div>
      `);
    ...
    if (!unique.length) {
      chunks.push(`
        <div class="empty-state mf-global-result">
          <h3>No reliable result found</h3>
          <p>
            MacroForge searched the global nutrition database
            for “${esc(cleaned)}” but did not find a product
            with a usable English name and nutrition data.
          </p>
          <button
            class="primary"
            onclick="modal('customModal')"
          >
            Add as custom food
          </button>
        </div>
    ...
    mfRenderSearchStatus(
      unique.length
        ? `Found ${unique.length} global result${unique.length === 1 ? "" : "s"} for “${cleaned}”.`
        : `No reliable global result for “${cleaned}”.`
    );
    ...
    box.insertAdjacentHTML(
      ...
      `
        <div class="empty-state mf-global-result">
          <h3>Global search could not connect</h3>
          <p>
            MacroForge tried the current full-text Open Food Facts
            search and its legacy fallback. Your local Pakistani
            database and Custom Food are still available.
          </p>
          <button
            class="secondary-btn"
            onclick="mfSearchGlobal(${JSON.stringify(cleaned)})"
          >
            Try again
          </button>
        </div>
